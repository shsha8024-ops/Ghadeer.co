...
function fmt(n) {
   ...
   try {
      ...
      return Number(n).toLocaleString(locale || 'en-US') + ' ' + (currency || 'USD');
   }
}
...
$('#addClientBtn').addEventListener('click', async () => {
   ...
   if (!name) 
      return alert('Enter client name / أدخل اسم العميل');
   if (!decryptedState.clients[name]) 
      decryptedState.clients[name] = {
         name, phone, city, ledger: []
   };
   else {
...
$('#exportClientsBtn').addEventListener('click', () => {
   const rows = Object.values(decryptedState.clients).map(c => ({
      name: c.name,
      phone: c.phone || '',
      city: c.city || '',
...
function renderClients() {
   ...
   if (q) {
      list = list.filter(c => (c.name || '').toLowerCase().includes(q) || (c.phone || '').toLowerCase().includes(q) || (c.city || '').toLowerCase().includes(q));
   }
   ...
   if (sort === 'invoices') 
      list.sort((a, b) => b.ledger.length - a.ledger.length);
   list.forEach(c => {
...
$('#addInvoiceBtn').addEventListener('click', async () => {
   ...
   if (!client) 
      return alert('Select client / اختر العميل');
   if (!(amount > 0)) 
      return alert('Enter valid amount / أدخل مبلغًا صحيحًا');
   decryptedState.clients[client].ledger.push({
      id: uid(),
      type: 'invoice',
      desc, amount, date
   });
   ...
   renderClients();
   renderSummary();
   alert('Invoice added / تمت إضافة الفاتورة');
});
$('#addPaymentBtn').addEventListener('click', async () => {
   ...
   renderClients();
   renderSummary();
   alert('Payment recorded / تم تسجيل الدفعة');
});
...
function renderSummary() {
   ...
   const ranked = all.map(c => {
      ...
      return {
         name: c.name,
         inv, count: c.ledger.filter(x => x.type === 'invoice').length,
         bal
      };
...
$('#d_export').addEventListener('click', () => {
   ...
   a.href = url;
   a.download = 'statement_' + safeFile(currentStatementName) + '.json';
   document.body.appendChild(a);
   a.click();
   a.remove();
   URL.revokeObjectURL(url);
...
$('#d_print').addEventListener('click', () => {
   if (!currentStatementName) 
      return alert('Open a client statement first / افتح كشف حساب عميل أولاً');
...
function openStatement(name) {
   ...
   document.getElementById('d_range').value = 'all';
...
function buildStatementRows(name, raw = false) {
   ...
   const range = document.getElementById('d_range').value;
   ...
   if (q) 
      list = list.filter(x => (x.desc || '').toLowerCase().includes(q));
   if (sinceDays) {
...
function renderStatement(name) {
   const tbody = document.getElementById('d_rows');
   tbody.innerHTML = '';
   if (!name) 
      return;
...
async function editOp(name, opId) {
   ...
   if (!op)
      return;
   const descInput = prompt('Description / الوصف:', op.desc || '');
   if (descInput === null)
      return;
   const amountInput = prompt('Amount / المبلغ:', op.amount);
   if (amountInput === null)
      return;
   const amountText = amountInput.trim();
   const parsedAmount = amountText === '' ? Number(op.amount) : Number(amountText);
   if (!Number.isFinite(parsedAmount) || !(parsedAmount > 0))
      return alert('Invalid amount / مبلغ غير صالح');
   const dateInput = prompt('Date (YYYY-MM-DD) / التاريخ:', op.date || today());
   if (dateInput === null)
      return;
   const newDesc = descInput.trim() || op.desc;
   const newDate = dateInput.trim() || op.date || today();
   op.desc = newDesc;
   op.amount = parsedAmount;
   op.date = newDate;
   await saveState();
   renderClients();
   renderStatement(name);
...
async function deleteOp(name, opId) {
   if (!confirm('Delete operation? حذف العملية؟')) 
      return;
   const c = decryptedState.clients[name];
